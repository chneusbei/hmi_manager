...
//    List<HashMap> getPressureCurveWithDateStart(@Param("handleDate") String handleDate);
    ...
    @Insert({"insert into pressure_curve (ID, PRESS_DATA_ID, RECORD_NO, POSITION, PRESS_FORCE, CUR_SPEED, HANDLE_DATE,PRESS_DATE, IS_DELETED, CREATE_BY, UPDATE_BY,CREATE_TIME,UPDATE_TIME) values(null, #{pressDataId}, #{recordNo}, #{position}, #{pressForce}, #{curSpeed}, #{handleDate}, #{pressDate}, '0', #{createBy}, #{createBy}, now(), now())"})
    void insert(@Param("entity") PressureCurveEntity entity);

    @Insert("<script>" +
            "insert into pressure_curve (ID, PRESS_DATA_ID, RECORD_NO, POSITION, PRESS_FORCE, CUR_SPEED, HANDLE_DATE,PRESS_DATE, IS_DELETED, CREATE_BY, UPDATE_BY,CREATE_TIME,UPDATE_TIME) " +
            "values " +
            "<foreach collection='entityList' item='entity' index='index' separator=','>" +
            "(null, #{entity.pressDataId}, #{entity.recordNo}, #{entity.position}, #{entity.pressForce}, #{entity.curSpeed}, #{entity.handleDate}, #{entity.pressDate}, '0', #{entity.createBy}, #{entity.createBy}, now(), now())" +
            "</foreach>" +
            "</script>")
